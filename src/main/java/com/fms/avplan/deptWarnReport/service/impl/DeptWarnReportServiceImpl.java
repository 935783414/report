package com.fms.avplan.deptWarnReport.service.impl;
...
@Xjava(id = "deptWarnReportService")
public class DeptWarnReportServiceImpl implements IDeptWarnReportService {

    private static Logger logger = Logger.getLogger(DeptWarnReportServiceImpl.class);


    @SuppressWarnings("unused")
    @Xjava
    private IDeptWarnReportDAO deptWarnReportDAO;

    @SuppressWarnings("unused")
    @Xjava
    private IDeptExamReportService deptExamReportService;

    @Override
    public List<DeptReport> queryList(DeptReport params) {
        Integer warnDays = params.getWarnDays();
        if (warnDays == 0) {//如果为0   则不做预警天数筛选
            params.setWarnDays(null);
        }

        List<DeptReport> deptReports = deptWarnReportDAO.queryList(params);

        Collections.sort(deptReports, new Comparator<DeptReport>() {
            private String[] warnLevel = {"高", "中", "低", "完成"};

            @Override
            public int compare(DeptReport o1, DeptReport o2) {
                int o1Index = Arrays.binarySearch(warnLevel, o1.getWarn_level());
                int o2Index = Arrays.binarySearch(warnLevel, o2.getWarn_level());

                int i = o1Index - o2Index;
                if (i == 0) {//如果预警等级相等 则按照任务完成度排序
                    double finish_percent1 = o1.getFinish_percent();
                    double finish_percent2 = o2.getFinish_percent();
                    return finish_percent1 > finish_percent2 ? 1 : (finish_percent1 == finish_percent2) ? 0 : -1;
                }
                return i;
            }
        });
    ...
    public void updateDeptReport(DeptReport report) {
        deptWarnReportDAO.updateDeptReport(report);
    }

    @Override
    public List<DeptReport> queryUnitDepts() {
        List<DeptReport> reportList = deptWarnReportDAO.queryUnitDepts();
    ...
    public int saveDatas() {
        return deptWarnReportDAO.saveDeptReport();
    }

    @Override
    public String exportDatas(DeptReport report) {
        return deptWarnReportDAO.exportDeptReport(report);
    }

    @Override
    public void deleteData(DeptReport report) {
        deptWarnReportDAO.deleteData(report);
    }

    @Override
    public List<DeptReport> queryReportDateList() {
        return deptWarnReportDAO.queryReportDateList();
    }

    @Override
    public Map<String, Map<String, Map<String, Object>>> queryReportReasonById(String innerid) {
        try {
            Map<String, Object> data = deptWarnReportDAO.queryByInnerid(innerid);
            if (data == null) {
...
//
            ...
            //获取整体的数据 开始
            Map<String, Map<String, Object>> totalMap = deptExamReportService.getTotalMap(select);
            baseMap.put("总体情况:" + orderno, totalMap);
            //获取整体的数据 结束
            Map<String, Object> detailData = deptWarnReportDAO.queryDetailReasons(innerid);
...
//            baseMap.put("工艺设计", designMap);
    ...
    public void reSaveDetailReport(String date) {
        this.deptWarnReportDAO.reSaveDetailReport(date);
    }
...
//            Date plan_start_time = DateUtils.parseDate(planstarttime,datePatterns);
    ...
    private void getMachingMap(Map<String, Map<String, Object>> machingMap, List<Map<String, Object>> maching) throws ParseException {
        ...
        for (Map<String, Object> result : maching) {
            ...
            String key = component + item_name + "（" + item_code + "）";
...
//                _partStatus.set(1,"success");
     ...
     * @return 总体情况数据
     */
    private Map<String, Map<String, Object>> getTotalMap(DeptReport select) {
        ...
        List<Map<String, String>> deptReportList = deptWarnReportDAO.queryAllDelayingReasonList(select);
