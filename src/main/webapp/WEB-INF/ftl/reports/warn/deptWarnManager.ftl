...
<div id="page-wrapper" style="padding-top: 10px;margin: 15px;">
    <div class="row">
        <div class="col-md-12">
            <h1 class="page-head-line" id="scaleName">预警管理</h1>
...
<#--toolbar-->
    <div id="toolbar" >
        <button type="button" class="btn btn-warning ladda-button" data-style="contract"  id="createWarnReport">生成当天预警</button>
    </div>
    <#--end toolbar-->
</div>
<script type="text/javascript">
    ...
    function formatterOperator(value, row, index) {
        ...
        var deleteBtn = "<div class='col-md-4'><a tabindex='0'  role='button' class='btn btn-sm btn-danger operator-delete ' data-html='true' " +
                ...
                " data-content='<div class=\"alert alert-danger text-center\" style=\"margin-bottom:5px;\" ><p>确认要删除该当天预警数据？</p><hr />" +
                ...
                " data-reportdate='"+row.reportdate+"'>&nbsp;删&nbsp;除&nbsp;预&nbsp;警&nbsp;</a></div>";
        ...
        var exportBtn = "<div class='col-md-4'><button type='button' class='btn btn-sm btn-info operator-export ladda-button'  data-style='contract' data-reportdate='"+row.reportdate+"'>&nbsp;导&nbsp;出&nbsp;预&nbsp;警&nbsp;</button></div>";
    ...
    $("#createWarnReport").click(function () {
        saveL.start();
        $.ajax({
            type: 'post',
            url: '${ctx}/warn/createWarnReport.z',
            contentType: "application/x-www-form-urlencoded",
            dataType: 'json',
    ...
    $("#manager-deptexam-table").on('click','.operator-detail',function (e) {
        ...
        form2.attr({"method":"post","action":'${ctx}/warn/deptWarn/admin.z'})
        form2.submit();
    ...
    $("#manager-deptexam-table").on('click','.operator-export',function (e) {
        ...
        form.append(input);
        form.attr({"method":"post","action":'${ctx}/report/deptexam/exportDatas.z'})
        var options = {
            url: '${ctx}/report/deptexam/exportDatas.z',
            type:'post',
            ...
            success:function (data) {
                if(data.success){
                    ...
                    form2.append(input2);
                    form2.appendTo(document.body);
                    form2.attr({"method":"post","action":'${ctx}/report/deptexam/download.z'})
                    form2.submit();
                    document.body.removeChild(form2[0]);
                }
                l.stop();
    ...
    var saveL = Ladda.create( document.querySelector( '#createWarnReport' ) );
